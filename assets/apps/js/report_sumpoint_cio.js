$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/report_sumpoint_cio/fetch_report_sumpoint_cio',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/report_sumpoint_cio/del_report_sumpoint_cio',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/report_sumpoint_cio/save_report_sumpoint_cio',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/report_sumpoint_cio/get_report_sumpoint_cio',
                       params = {
                           id: id
                       }
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#kpi_name").val(data.rows["kpi_name"]);$("#kpi_id").val(data.rows["kpi_id"]);$("#kpi_type").val(data.rows["kpi_type"]);$("#month_time").val(data.rows["month_time"]);$("#amp").val(data.rows["amp"]);$("#target").val(data.rows["target"]);$("#result").val(data.rows["result"]);$("#calc_result").val(data.rows["calc_result"]);$("#point").val(data.rows["point"]);$("#kpi_name").val(data.rows["kpi_name"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.kpi_name=$("#kpi_name").val();items.kpi_id=$("#kpi_id").val();items.kpi_type=$("#kpi_type").val();items.month_time=$("#month_time").val();items.amp=$("#amp").val();items.target=$("#target").val();items.result=$("#result").val();items.calc_result=$("#calc_result").val();items.point=$("#point").val();items.kpi_name=$("#kpi_name").val();
